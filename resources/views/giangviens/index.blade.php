    ...
    <tbody>
        @foreach ($giangviens as $giangvien)
            <tr>
                ...
                <td>{{ $giangvien->email }}</td>
                <td>
                    <form action="{{ route('giangviens.edit', $giangvien->id) }}" method="GET" style="display:inline;">
                        @csrf
                    ...
                    <form action="{{ route('giangviens.destroy', $giangvien->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-info my-3" onclick="return confirm('Xóa Giảng Viên này ?')">
